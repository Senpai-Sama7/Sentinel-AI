...
async def test_health_check(async_test_app_client: AsyncClient):
    ...
    assert "redis" in data and "chroma" in data


@pytest.mark.asyncio
async def test_metrics_endpoint(async_test_app_client: AsyncClient):
    response = await async_test_app_client.get("/metrics")
    assert response.status_code == 200
    assert b"documents_ingested_total" in response.content


@pytest.mark.asyncio
async def test_get_file_success(
    async_test_app_client: AsyncClient, mock_memory_manager: AsyncMock
):
...
async def test_get_file_not_found(
    ...
    mock_memory_manager.get_file_content.side_effect = NotFoundError(
...
@pytest.mark.asyncio
async def test_get_file_memory_layer_failure(
    async_test_app_client: AsyncClient, mock_memory_manager: AsyncMock
):
    """Tests the 503 response when a backend service is unavailable."""
    file_path = "src/main.py"
    # Configure the mock to raise a service error
    mock_memory_manager.get_file_content.side_effect = MemoryLayerError(
        "L3-Git", "Repository is corrupted."
    )
